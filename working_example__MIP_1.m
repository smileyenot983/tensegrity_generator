...
% working seeds: 11,18,22



strut_length_constraint = false;
strut_max_length = 15; 

cable_length_constraint = false;
cable_max_length = 15; 

max_proj_constraint=false;
max_dir_length = 10;
dir_constraint = 'no'; 
%% GRID
...
%     sum(P(:)) >= 45;
    ...
    if strut_length_constraint==true
        for i=1:n        
    ...
    if cable_length_constraint==true
        for i=1:n
            for j = 1:n
    ...
    if max_proj_constraint
        if dir_constraint=='x'
            dir=x;
        elseif dir_constraint=='y'
            dir=y;
        elseif dir_constraint=='z'
            dir=z;
        else
            dir=[0;0;0];
        end
    ...
    %     l_0 = 2.5;
        for i=1:n
            norm(  (sum(repmat(R(i,:),3,1).*points,2) -points(:,i))'*y )<=max_dir_length;
        end
...
dataname = strcat(grid_structure,"_",exp_num,"_","c_constr:",string(cable_length_constraint),"_","s_constr:",string(strut_length_constraint),"_",".mat");
mkdir(foldername);
